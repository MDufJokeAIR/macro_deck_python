...
from macro_deck_python.models.action_button import ActionButton, ActionSlider, SliderCell
...
class MacroDeckServer:
    ...
    async def _on_button_press(self, info: ClientInfo, msg: dict) -> None:
        ...
        folder = _find_folder(profile.folder, folder_id) or profile.folder

        cell = None
        if position:
            cell = folder.buttons.get(position)
        if cell is None and button_id:
            cell = next((b for b in folder.buttons.values()
                         if b.cell_id == button_id), None)

        if cell is None:
            await info.ws.send(encode("ERROR", message="Button not found"))
            return

        # Sliders and their occupied cells are NOT pressable — they send
        # SET_VARIABLE directly from the client drag gesture.
        if isinstance(cell, (ActionSlider, SliderCell)):
            return

        # At this point cell is definitely an ActionButton
        if cell.state_binding is not None:
            pass   # state driven by variable-change hook

        await self._broadcast(
            encode("BUTTON_STATE", button_id=cell.cell_id, state=cell.state))
        execute_button(cell, info.client_id)
        await self._send_buttons(info)
    ...
    async def _send_buttons(self, info: ClientInfo, folder_id: Optional[str] = None) -> None:
        ...
        def _button_payload(pos: str, cell) -> Optional[dict]:
            row, col = pos.split("_")
            base = {"button_id": cell.cell_id, "position": pos,
                    "row": int(row), "col": int(col)}

            if isinstance(cell, SliderCell):
                # Occupied cell — client skips it; we still send it so the
                # client knows not to render an empty button there.
        ...
        buttons_payload = [p for p in
                           (_button_payload(pos, cell)
                            for pos, cell in folder.buttons.items())
                           if p]

        sub_folders = [{"folder_id": sf.folder_id, "name": sf.name}
                       for sf in folder.sub_folders]

        # Legacy SliderWidget occupancy (kept for backward compat)
        slider_cells: dict = {}
