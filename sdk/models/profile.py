...
from macro_deck_python.models.action_button import (
    ActionInterface, ActionButton, ActionSlider, SliderCell,
)
...
class Folder:
    ...
    name: str = "Main"
    # key = "row_col" string; values are ActionButton, ActionSlider, or SliderCell
    buttons: Dict[str, ActionInterface] = field(default_factory=dict)
    sub_folders: List["Folder"] = field(default_factory=list)
    # legacy SliderWidget registry (kept for backward compat)
    sliders: Dict[str, SliderWidget] = field(default_factory=dict)
    columns: int = 8
    rows: int = 4

    # ------------------------------------------------------------------
    def get_button(self, row: int, col: int) -> Optional[ActionInterface]:
        return self.buttons.get(f"{row}_{col}")

    def set_button(self, row: int, col: int, btn: ActionInterface) -> None:
    ...
    def from_dict(d: dict) -> "Folder":
        ...
        # ActionInterface.from_dict() dispatches to the right subclass automatically
        f.buttons     = {k: ActionInterface.from_dict(v)
                         for k, v in d.get("buttons", {}).items()}
        f.sub_folders = [Folder.from_dict(sf) for sf in d.get("sub_folders", [])]
        f.sliders     = {k: SliderWidget.from_dict(v)
                         for k, v in d.get("sliders", {}).items()}
        return f
