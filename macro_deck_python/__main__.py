...
# Add parent directory to sys.path to import root-level modules
...
from services.variable_manager import VariableManager
from utils.logger import MacroDeckLogger
...
async def _main_async(args: argparse.Namespace) -> None:
    ...
    # 9. WebSocket server
    from websocket.server import MacroDeckServer
    ...
    if not getattr(args, "no_gui", False) and ConfigManager.get("gui_enabled", True):
        try:
            ...
            from gui.web_config import create_app
    ...
    if not getattr(args, "no_hot_reload", False):
        from services.hot_reload import HotReloadWatcher
